...
import controllers.PasswordHasherMD5
...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  override def configure() {
    ...
    bind[UnsecuredErrorHandler].to[WvgUnsecuredErrorHandler]
    bind[SecuredErrorHandler].to[WvgSecuredErrorHandler]

    bind[UserService].to[UserServiceImpl]
    bind[UserDAO].to[UserDAOLdapImpl]
    ...
    bind[DelegableAuthInfoDAO[PasswordInfo]].to[PasswordInfoDAOLdapImpl]
  ...
  def providePasswordHasherRegistry(): PasswordHasherRegistry = {
    PasswordHasherRegistry(new PasswordHasherMD5(), Seq(new PasswordHasherMD5()))
  }
